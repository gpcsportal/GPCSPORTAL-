...
## Code quality checks
`.github/workflows/code-quality.yml` runs on pull requests to `main` and pushes to `main`. It validates Composer metadata, installs the locked dependencies, runs the Composer security audit, checks PHP and JavaScript syntax, boots Laravel routes, runs migrations against an isolated SQLite database, and executes the full test suite.
